...
class MmrHistory
{
    ...
    static getGamesAndAverageMmrString(mmrHistory)
    {
        ...
        const gamesMmr = MmrHistory.getGamesAndAverageMmr(mmrHistory);
        result += MmrHistory.getGamesAndAverageMmrEntryString(gamesMmr, "all");
        for(const race of Object.values(RACE))
            result += MmrHistory.getGamesAndAverageMmrEntryString(gamesMmr, race.name);
        return result;
    }
    ...
    {
        const firstDate = MmrHistory.calculateFirstMmrDate();
        const injected = [];
        //use the same datetime to correctly group the points by timestamp later
        const now = new Date();
        MmrHistory.injectLatestTeamMmrSnapshots(historyByRace, teams, queueFilter, teamTypeFilter, injected, firstDate);
        for(const raceHistory of historyByRace.values()) {
            MmrHistory.injectHistoryHeader(raceHistory, injected, firstDate);
            MmrHistory.fillMmrGaps(raceHistory, injected, now);
            raceHistory.sort((a, b)=>a.teamState.dateTime.getTime() - b.teamState.dateTime.getTime());
            MmrHistory.injectHistoryTail(raceHistory, injected, now);
        }
    ...
    static injectLatestTeamMmrSnapshots(racialHistory, teams, queueFilter, teamTypeFilter, injectArray, firstDate)
    {
        ...
        if(queueFilter == TEAM_FORMAT._1V1.code)
        {
            for(const race of Object.values(RACE))
            {
                ...
                const snap = MmrHistory.createTeamSnapshot(teamsRacial[0], firstDate, true);
    ...
    static injectHistoryHeader(history, injectArray, firstDate)
    {
        if(history.length == 0
        ...
        const snap = MmrHistory.cloneMmrPoint(history[0], firstDate);
    ...
    static fillMmrGaps(history, injected, now)
    {
        ...
        for(let i = 0; i < history.length; i++)
        {
            ...
            MmrHistory.injectMmrPoints(history, curInjected, prev, toInject);
        }
        Array.prototype.push.apply(injected, curInjected);
        Array.prototype.push.apply(history, curInjected);
    }

    static injectHistoryTail(history, injected, now)
    {
        const curInjected = [];
        MmrHistory.injectMmrPoints(history, curInjected, history[history.length - 1],
        ...
        if(lastPoint.teamState.dateTime.getTime() < lastPointMaxDateTime.getTime())
             curInjected.push(MmrHistory.cloneMmrPoint(lastPoint, lastPointMaxDateTime));
        Array.prototype.push.apply(injected, curInjected);
        Array.prototype.push.apply(history, curInjected);
    }
    ...
    static injectMmrPoints(history, injectArray, refPoint, toInject)
    {
        ...
        for(let ii = 0; ii < toInject; ii++)
        {
            ...
            date.setMinutes(0);
            date.setSeconds(0, 0);
            const point = MmrHistory.cloneMmrPoint(refPoint, date);
    ...
    static createHistoryGamesFromTeamState(curData)
    {
        return MmrHistory.createHistoryGames(curData.teamState.games, curData.teamState.wins);
    }

    static appendAdditionalHistoryRanks(curData, lines)
    {
        lines.push(MmrHistory.createHistoryRank(curData.teamState.globalRank, curData.teamState.globalTeamCount));
        lines.push(MmrHistory.createHistoryRank(curData.teamState.regionRank, curData.teamState.regionTeamCount));
        lines.push(MmrHistory.createHistoryRank(curData.teamState.leagueRank, curData.teamState.leagueTeamCount));
    }
...
MmrHistory.TEAM_SNAPSHOT_SEASON_END_OFFSET_MILLIS = 2 * 24 * 60 * 60 * 1000;
MmrHistory.MMR_Y_VALUE_GETTERS = new Map([
