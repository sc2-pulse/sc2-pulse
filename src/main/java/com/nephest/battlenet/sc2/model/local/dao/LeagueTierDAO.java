...
public class LeagueTierDAO
{
    ...
    private static final String MERGE_QUERY =
        ...
        + "selected AS "
        + "("
            + "SELECT league_tier.id, "
            + "league_tier.league_id, "
            + "league_tier.type "
            + "FROM league_tier "
            + "INNER JOIN vals v(league_id, type, min_rating, max_rating) "
                + "ON league_tier.league_id = v.league_id "
                + "AND COALESCE(league_tier.type, -1) = COALESCE(v.type::smallint, -1) "
        + "), "
        + "updated AS "
        + "("
            + "UPDATE league_tier "
            + "SET min_rating = v.min_rating::smallint, "
            + "max_rating = v.max_rating::smallint "
            + "FROM selected "
            + "INNER JOIN vals v(league_id, type, min_rating, max_rating) "
                + "ON selected.league_id = v.league_id "
                + "AND COALESCE(selected.type, -1) =  COALESCE(v.type::smallint, -1) "
        ...
        + "("
            ...
            + "min_rating::smallint, "
            + "max_rating::smallint "
            + "FROM vals v(league_id, type, min_rating, max_rating) "
