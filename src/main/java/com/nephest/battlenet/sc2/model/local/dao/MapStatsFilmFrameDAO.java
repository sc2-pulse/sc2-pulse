...
public class MapStatsFilmFrameDAO
{
    ...
    private static final String ADD =
        ...
        + "(\n"
            ...
            + "region,\n"
            + "season,\n"
            + "league_tier[1] AS league_tier,\n"
            + "duration_ix,\n"
            + "decision\n"
            + "FROM match_group\n"
            + "WHERE matchup[1] != matchup[2]\n"
            + "AND (league_tier[1]).id = (league_tier[2]).id\n"
        + "),\n"
        + "stats_group AS\n"
        + "(\n"
            + "SELECT map_id, matchup, region, season, (league_tier).league, (league_tier).tier, duration_ix,\n"
            + "COUNT(*) as games,\n"
            + "COUNT(*) FILTER(WHERE decision[1] = matchup[1]) as wins\n"
            + "FROM match_group_filter\n"
            + "GROUP BY map_id, matchup, region, season, league_tier, duration_ix\n"
        + "),\n"
        + "film_group AS\n"
        + "(\n"
            + "SELECT map_id,\n"
        ...
        + "(\n"
            ...
            + "film_group.*\n"
            + "FROM film_group\n"
            + "INNER JOIN map_stats_film USING(map_id, league_tier_id, map_stats_film_spec_id)\n"
        + "),\n"
        + "new_film AS\n"
        + "(\n"
            + "INSERT INTO map_stats_film(map_id, league_tier_id, map_stats_film_spec_id)\n"
            + "SELECT DISTINCT ON(map_id, league_tier_id, map_stats_film_spec_id)\n"
            + "map_id, league_tier_id, map_stats_film_spec_id\n"
            + "FROM film_group\n"
            + "WHERE NOT EXISTS\n"
            + "(\n"
                + "SELECT 1\n"
        ...
        + "(\n"
            ...
            + "film_group.*\n"
            + "FROM film_group\n"
            + "INNER JOIN new_film USING(map_id, league_tier_id, map_stats_film_spec_id)\n"
        + "),\n"
        + "all_film_group AS\n"
        + "(\n"
