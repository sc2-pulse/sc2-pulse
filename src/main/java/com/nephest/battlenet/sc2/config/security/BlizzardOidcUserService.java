...
implements Oauth2UserServiceRegistration<OidcUserRequest, OidcUser>
{
     ...
     *     <li>If not found, then merge the account and Blizzard characters.</li>
     * </ul>
    ...
    private Account findOrMerge(OidcUser user)
    {
        ...
        Long blizzardAccountId = Long.valueOf(user.getSubject());
        return accountDAO.find(partition, battleTag)
            .orElseGet(()->mergeByBlizzardCharacters(partition, battleTag, blizzardAccountId));
    }

    private Account mergeByBlizzardCharacters
    (
        Partition partition,
        String battleTag,
    ...
    {
        List<BlizzardFullPlayerCharacter> bChars = WebServiceUtil.getOnErrorLogAndSkipFlux(
                api.getPlayerCharacters(Region.EU, blizzardAccountId))
            .collectList()
            .block();
        return findByBlizzardCharacters(bChars)
            .orElseGet(()->mergeAccountAndBlizzardCharacters(bChars, partition, battleTag));
    }

    private Optional<Account> findByBlizzardCharacters
    (
        Collection<BlizzardFullPlayerCharacter> blizzardCharacters
    )
    {
        return blizzardCharacters.stream()
    ...
    private Account mergeAccountAndBlizzardCharacters
    (
        Collection<BlizzardFullPlayerCharacter> blizzardCharacters,
        Partition partition,
        String battleTag
    ...
    {
        Account newAccount = accountDAO.merge(new Account(null, partition, battleTag));
        blizzardCharacters.stream()
            .map(bChar->PlayerCharacter.of(newAccount, bChar.getRegion(), bChar))
